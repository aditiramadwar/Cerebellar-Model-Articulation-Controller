...
num_of_assoc_vec = num_weights - gen_factor + 1

# Normalize the input data to create the active association cells for each input,
# this function gives ou the index of in the range of 35 of where the active cells should start from for 
# each input. 
...
def get_association_map(data):
    ...
    for data_val in (data):
        activation_idx = (num_of_assoc_vec-2) * ((data_val - min_val)/(max_val - min_val)) + 1
        # high = int(math.ceil(activation_idx))
        # low = int(math.floor(activation_idx))
        association_matrix[data_val] = (int(math.ceil(activation_idx)), int(math.floor(activation_idx)))
...
def train(train_data, des_data, epochs = 10000, learning = 0.1):
    ...
    for i in range(epochs):
        ...
        for ind, data_val in enumerate(train_data):
            right, left = association_matrix[data_val]
            # 20% of prev, 80% of curr
            y_output_right = np.sum(weight_array[right : right + gen_factor])
            y_output_left = np.sum(weight_array[left : left + gen_factor])
            y_output = y_output_right*0.2 + y_output_left*0.8
            error =  des_data[ind] - y_output
            out = np.append(out, y_output)
            correction = (learning * error)/gen_factor
            weight_array[right : (right + gen_factor)] = [(weight_array[ind] + correction) for ind in range(right, (right + gen_factor))]
            weight_array[left : (left + gen_factor)] = [(weight_array[ind] + correction) for ind in range(left, (left + gen_factor))]
...
def test(test_data, test_res_des, weights):
    ...
    for data_val in (test_data):
            right, left = test_assoc[data_val]
            y_output_right = np.sum(weight_array[right : right + gen_factor])
            y_output_left = np.sum(weight_array[left : left + gen_factor])
            # 20% of prev, 80% of curr
            y_output = y_output_right*0.2 + y_output_left*0.8
...
plt.xlabel('x')
plt.ylabel('x^2')
plt.savefig('result_continuous.png')
plt.show()
